...
OptionParser.parse do |parser|
  ...
  parser.on(
    "-n NAME",
    "--name=NAME",
    "Sets the display name of the app process (default name: #{Sentry::Config.shard_name})") { |shard_name| cli_config.display_name = shard_name }
  parser.on(
    "-b COMMAND",
    "--build=COMMAND",
  ...
  parser.on(
    "--ameba",
    "Run 'ameba' once before running Sentry build and run commands") do
    cli_config.run_ameba = true
  end
  parser.on(
    "--no-color",
...
run_ameba = config.run_ameba
config.merge!(cli_config)
config.run_ameba = true if run_ameba
